...
import './Navbar.css'
function Navbar(){
    const {displayNavbar,setDisplayNavbar, resetCreateRoutine, currentPage, setCurrentPage, setIsDarkMode, isDarkMode} = useContext(GeneralContext)
         ...
         onClick={(event)=>hideNavbar(event.target.className)}>
            ...
            className='Navbar-container'>
                ...
                <div className='mode-cont'>
                    <p>Dark mode</p>
                    <div className='track'>
                        <div className={`thumb ${isDarkMode && 'on'}`} 
                         onClick={()=>handleChangeMode()}></div>
                    </div>
                </div>
            </div>
        </aside>
